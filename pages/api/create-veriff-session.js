...
const handler = async (req, res) => {
  if (req.method === 'POST') {
    upload.single('file')(req, res, async (err) => {
      ...
      const clientSecret = process.env.VERIFF_SHARED_SECRET; // Make sure to set this in your .env.local

      if (!clientAuthToken || !clientSecret) {
        console.error('API key or shared secret is missing');
        return res.status(500).json({ error: 'API key or shared secret is missing' });
      }
      ...
      const verificationRequestData = {
        verification: {
          ...
          document: {
            ...
            country: 'US', // Update country code as per requirement
          },
        },
      };
